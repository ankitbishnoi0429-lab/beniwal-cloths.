...
app = Flask(__name__)
app.secret_key = os.environ.get('SECRET_KEY', 'devkey@123secure-change-in-production')
...
try:
    if GOOGLE_API_KEY:
        ...
        print("WARNING: GOOGLE_API_KEY not set. AI features disabled.")
        AI_ENABLED = False
...
def index():
    try:
        products = query_db("SELECT p.*, s.name as seller_name, s.rating as seller_rating, s.lat as seller_lat, s.lon as seller_lon FROM products p LEFT JOIN sellers s ON p.seller_id = s.id")
    except:
        # Fallback if sellers table is empty
        products = query_db("SELECT * FROM products")
    
    # Convert sqlite3.Row to dict and parse images
    result = []
    for product in products:
        prod_dict = dict(product)
        try:
            prod_dict['images_list'] = json.loads(prod_dict.get('images', '[]')) if prod_dict.get('images') else []
        except:
            prod_dict['images_list'] = []
